<script>
$(function() {
  $( "#accordion" ).accordion({
    collapsible: true,
    active: false,
    heightStyle: "content"
  });
});
</script>

<h2>Description des missions</h2>

<div id="accordion">
  <h3>Installation de la salle<span class="effectif">1 / 3</span></h3>
  <div>
    <div class="icon fa-clock-o">&nbsp;la veille au soir&nbsp;</div>
    <div class="icon fa-users">&nbsp;15&nbsp;</div>
    <div class="icon fa-star">&nbsp;&nbsp;</div>
    <p>Positionner les chaises, les barrières, les tapis, la sono ...</p>
  </div>

  <h3>Buvette</h3>
  <div>
    <div class="icon fa-clock-o">&nbsp;permanence toute la journée, rush entre 11h30 et 13h30&nbsp;</div>
    <div class="icon fa-users">&nbsp;5 ; compatible pointage&nbsp;</div>
    <div class="icon fa-star">&nbsp;Qui veut être le roi du hot-dog&nbsp;?&nbsp;</div>
    <p>Tenir le stand buvette, confectionner les sandwiches, les crêpes ...</p>
  </div>

  <h3>Pointage des compétiteurs</h3>
  <div>
    <div class="icon fa-clock-o">&nbsp;8h30-9h15 puis 11h-12h&nbsp;</div>
    <div class="icon fa-users">&nbsp;5 ; compatible buvette&nbsp;</div>
    <div class="icon fa-star">&nbsp;&nbsp;</div>
    <p>Vérifier les licences, remettre les dossards ...</p>
  </div>

  <h3>Juges de blocs</h3>
  <div>
    <div class="icon fa-clock-o">&nbsp;9h30-12 et/ou 12h30-15h&nbsp;</div>
    <div class="icon fa-users">&nbsp;30 (+formation préalable)&nbsp;</div>
    <div class="icon fa-star">&nbsp;Fabien&nbsp;</div>
    <p>Organiser le passage des jeunes dans le bloc dont vous êtes responsable, observer et inscrire leur prestation sur les feuilles de route.</p>
  </div>

  <h3>Saisie des résultats</h3>
  <div>
    <div class="icon fa-clock-o">&nbsp;12h-13h30 puis 15h-16h30&nbsp;</div>
    <div class="icon fa-users">&nbsp;4 (+formation préalable)&nbsp;</div>
    <div class="icon fa-star">&nbsp;Romain/Viviane &nbsp;</div>
    <p>Saisir informatiquement les résultats, en binôme. Concentration et rigueur sont de mise !</p>
  </div>

  <h3>Photos &amp; Vidéos</h3>
  <div>
    <div class="icon fa-clock-o">&nbsp;toute la journée est à couvrir&nbsp;</div>
    <div class="icon fa-users">&nbsp;mini 1 photo + 1 vidéo &nbsp;</div>
    <div class="icon fa-star">&nbsp;Grégoire&nbsp;</div>
    <p>Couvrir l'évènement. Les photographes "officiels" pourront avoir accès aux zones propices à la prise de vue.</p>
  </div>

  <h3>Rangement de la salle</h3>
  <div>
    <div class="icon fa-clock-o">&nbsp;À partir de 16h30&nbsp;</div>
    <div class="icon fa-users">&nbsp;15&nbsp;</div>
    <div class="icon fa-star">&nbsp;&nbsp;</div>
    <p>... et après on boit un coup ? ;-)</p>
  </div>
</div>
